...
function emToPixels(l, n) {
  n = n || document.documentElement;
  var c = getComputedStyle(n).fontSize;
  c || (c = getComputedStyle(document.documentElement).fontSize);
  var e = parseFloat(c),
    a = l * e;
  return a;
}
function copyStyles(l, n) {
  const c = window.getComputedStyle(l);
  for (let e = 0; e < c.length; e++) {
    const a = c[e];
    a == 'width' && (n.style[a] = c.getPropertyValue(a));
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function c(e) {
    ...
    let g = e.style.marginBottom,
      b = emToPixels(1.5, t);
    const d = document.createElement('div');
    e.tagName == 'TEXTAREA'
      ? b < parseFloat(t.style.height) && (t.style.height = '1.5em')
      : (b >= parseFloat(t.style.height) &&
          ((a = 0),
          (e.style.paddingTop = 0),
          (e.style.paddingBottom = 0),
          (d.style.lineHeight = 0)),
        (t.style.height = e.offsetHeight - a * 2 + 'px'));
    let y = emToPixels(2, t);
    (e.style.paddingRight || 0 < y) && (e.style.paddingRight = y + 'px'),
      (t.style.bottom = a + g + 'px'),
      (d.className = 'micro-ai-wrapper'),
      copyStyles(e, d),
      (e.style.width = '100%'),
      (e.style.height = '100%'),
    ...
    function x(p, m) {
      m.forEach(function (s) {
        ...
        if (
          ((u.innerHTML = s.html),
          (u.className = 'micro-ai-menu-item'),
          s.type == 'group' && (u.className += ' group'),
          u.addEventListener('click', async function () {
            ...
            switch (s.type) {
              case 'translate':
                r = `${BASE_URL}?action=translate&lang=${M}&content=${encodeURIComponent(
                  o
                )}`;
                break;
              case 'change_tone':
                r = `${BASE_URL}?action=change_tone&tone=${S}&content=${encodeURIComponent(
                  o
                )}`;
                break;
              case 'shorter':
                r = `${BASE_URL}?action=make_shorter&content=${encodeURIComponent(
                  o
                )}`;
                break;
              case 'longer':
                r = `${BASE_URL}?action=make_longer&content=${encodeURIComponent(
                  o
                )}`;
                break;
              case 'spelling':
                r = `${BASE_URL}?action=fix_spelling&content=${encodeURIComponent(
                  o
                )}`;
                break;
              case 'lucky':
                r = `${BASE_URL}?action=lucky&content=${encodeURIComponent(o)}`;
                break;
              default:
                break;
            }
            if (r)
              try {
                r += '&location=' + encodeURIComponent(window.location.href);
                ...
                if (
                  ...
                  alert(v.error), console.error('Error: ', v.error);
                  return;
                }
                ...
                i.ok
                  ...
                  : console.error('Error fethcing data: ', f);
              } catch (i) {
    ...
    x(h, w),
      ...
      t.addEventListener('click', function () {
        let p = h.className.split(' ');
        const m = p.findIndex((s) => s == 'show');
        m >= 0 ? p.splice(m, 1) : p.push('show'), (h.className = p.join(' '));
      }),
      document.addEventListener('click', function (p) {
        d.contains(p.target) || n(h);
      });
  }
  setInterval(() => {
    document.querySelectorAll('input[type="text"], textarea').forEach(c);
  }, 500);
});
